...
def get_loss(sess,net,img_ref,layer):
	...
	for layer, _ in list_numberFeaturesOutput:
		x = net[layer]
...
def main(args):
	...
	net['input'] = current
	numberChannelsTemp = numberChannels
	for name, numberFeaturesOutput in list_numberFeaturesOutput:
		kernel = tf.constant(np.random.uniform(low=-1,high=1,size=(3, 3, numberChannelsTemp,numberFeaturesOutput)),dtype=np.float32)
		conv = tf.nn.conv2d(current, kernel, strides=(1, 1, 1, 1),padding='SAME',name='conv')
		bias = tf.constant(np.random.uniform(low=-1,high=1,size=(numberFeaturesOutput)),dtype=np.float32)
		numberChannelsTemp = numberFeaturesOutput
		conv_add_bias = tf.nn.bias_add(conv, bias)
		current = conv_add_bias
		net[name] = current
